  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-900">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/95 px-5 py-3 backdrop-blur">
        ...
        <div className="flex items-center gap-2"><button onClick={() => setLocation('/?workspace=self')} className="rounded-lg bg-blue-600 px-3 py-2 text-sm text-white hover:bg-blue-700">开始作业</button><button onClick={() => logout()} className="rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-50 flex items-center gap-2"><LogOut className="w-4 h-4" />退出</button></div>
      </header>
      <div className="mx-auto max-w-7xl p-5 lg:p-8 grid gap-6 lg:grid-cols-[360px_1fr]">
        ...
        <section className="space-y-6">
          ...
          <section className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm"><div className="flex items-center gap-2 px-5 py-4 border-b border-slate-200 font-semibold"><Users className="w-4 h-4 text-blue-600" />账号与作业状态</div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-50 text-slate-500"><tr><th className="px-5 py-3 text-left">账号</th><th className="px-3 py-3 text-left">状态</th><th className="px-3 py-3 text-right">音频</th><th className="px-3 py-3 text-right">片段</th><th className="px-5 py-3 text-left">最近作业</th><th className="px-5 py-3 text-right">操作</th></tr></thead><tbody>{accounts.map(account => <tr key={account.id} className="border-t border-slate-100"><td className="px-5 py-3"><button type="button" onClick={() => account.role === "admin" ? setLocation('/?workspace=self') : setLocation(`/?workspace=${account.id}`)} className="text-left hover:text-blue-600"><div className="font-medium">{account.name || account.username}</div><div className="font-mono text-xs text-slate-500">{account.username}</div></button></td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs ${account.isActive ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{account.role === "admin" ? "管理员" : account.isActive ? "可作业" : "已停用"}</span></td><td className="px-3 py-3 text-right font-mono">{account.audioCount}</td><td className="px-3 py-3 text-right font-mono">{account.segmentCount}</td><td className="px-5 py-3 text-xs text-slate-500">{formatTime(account.latestWorkAt)}</td><td className="px-5 py-3 text-right"><div className="inline-flex gap-3"><button onClick={() => account.role === "admin" ? setLocation('/?workspace=self') : setLocation(`/?workspace=${account.id}`)} className="text-xs text-blue-600 hover:underline">{account.role === "admin" ? "开始作业" : "查看作业"}</button>{account.role !== "admin" && <><button onClick={() => renameWorker(account.id, account.username)} className="text-xs text-blue-600 hover:underline">修改账号</button><button onClick={() => resetWorkerPassword(account.id, account.username)} className="text-xs text-blue-600 hover:underline">重置密码</button><button onClick={() => updateWorker.mutate({ userId: account.id, isActive: !account.isActive })} className="text-xs text-blue-600 hover:underline">{account.isActive ? "停用" : "启用"}</button></>}</div></td></tr>)}</tbody></table>{accounts.length === 0 && <div className="p-8 text-center text-sm text-slate-500">尚无账号</div>}</div></section>
